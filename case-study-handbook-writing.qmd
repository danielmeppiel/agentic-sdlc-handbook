---
title: "Writing a ~68,000-Word Book with Agent Fleets"
...
%%| fig-alt: "Flowchart showing three process gaps that triggered dynamic persona creation. First path: Integration Review where editor reads all 15 chapters leads to a decision node asking if visual anchors are missing. Yes leads to creating the Illustrator Agent (Visual Strategist persona), which produces a fleet of 2 agents finding 40 visual opportunities and embedding 25 Mermaid diagrams. Second path: Integration Review leads to a decision node asking if inconsistent statistics exist across chapters. Yes leads to creating the Fact-Ref-Checker (Claims Auditor persona), which produces a fleet of 2 agents finding 75 flags including 5 critical ones. Third path: The question 'How to publish?' leads to a decision node asking if publishing expertise is needed. Yes leads to creating the Publishing Advisor, which evaluates 5 paths and recommends Open Core plus Premium model."
