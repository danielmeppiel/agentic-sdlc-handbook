...
::: {.callout-note title="A note on Part III — the architectural threshold"}
Part III is one author's first honest map of the other side of **the
architectural threshold**: the size and coupling at which "write more
careful prose" stops scaling and design discipline takes over. The
chapters teach the discipline. **Genesis is the tool I wished existed**
when I started — the same disciplines packaged as an Agent Skill the
harness already knows how to load. With it loaded, the chapters' pattern
names resolve to a companion the agent can read on disk:

```bash
apm install danielmeppiel/genesis
```

Works across Copilot, Claude Code, Cursor, Codex, and OpenCode via the
agentskills.io entrypoint contract. The book stands without it; the
later chapters' Genesis footnotes are callbacks to this companion, not
fresh introductions.
:::
