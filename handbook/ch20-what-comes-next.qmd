...
These five properties map directly to the PROSE constraints from Chapter 11. The constraints were not designed for today's models; they were designed for the fundamental properties of human-AI collaboration.
...
Pick the module your team changes most frequently. Not the biggest module, the most-changed one. Run the methodology from Chapter 10: identify implicit knowledge, undocumented conventions, architectural decisions that exist only in your team's memory. Write down what you find. You are not fixing anything today. You are measuring the gap between what an agent can see and what your team knows.
...
Take the top three conventions from yesterday's audit. Write each as an instruction file — one organizational standard, one architectural constraint, one domain-specific rule. Follow the format from Chapter 10, under the constraints from Chapter 11: scoped, testable, specific. Do not try to document everything. Three primitives that cover the most common mistakes are worth more than thirty that cover edge cases.
...
Review yesterday's comparison honestly. Which files made a difference? Which were ignored or misinterpreted by the agent? Revise the ones that didn't land. This is the calibration loop from Chapter 13: context files are not documentation, they are engineering artifacts that need testing and iteration like any other code.
...
The week described above is the human-side ramp. The agent-side ramp is one install.

`apm install danielmeppiel/genesis` puts an architectural layer for agentic systems on disk in the form your harness already loads — `.github/skills/genesis/` for Copilot, the equivalent path for Claude Code, Cursor, Codex, OpenCode.[^ch20-genesis] What it gives you is the operational version of every discipline this part argued: the load-lifecycle vocabulary made executable, the PROSE constraints encoded as primitives the agent reads on demand, the Rosetta Stone's pattern names available at the moment a design decision is in front of you. The handbook teaches the discipline; Genesis names the implementation the agent loads while you work through it.

This is not a dependency on the book's part — every chapter stands without it — and it is not the only such companion the field will produce. It is a worked example of what the practice looks like packaged as a primitive set, written by the same hand. Install it Monday morning, read this part with it loaded, and when a chapter names a pattern (Wave, Panel, Supervised Execution), grep the skill for the same name. The agent will surface the operational form. Use whichever name lands first in the conversation.
...
[^ch20-genesis]: `danielmeppiel/genesis`, https://github.com/danielmeppiel/genesis. Open-source Agent Skill packaging the architectural layer for agentic systems — composition substrate, design patterns, runtime affordances — across AGENTS.md-compatible harnesses (Copilot, Claude Code, Cursor, Codex, OpenCode). Per the directionality contract of this book, Genesis appears here as the agent-side reference companion, not as load-bearing structure for the chapters' arguments.
