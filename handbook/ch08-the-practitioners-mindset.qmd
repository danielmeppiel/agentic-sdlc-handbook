...
# The Practitioner's Mindset {#sec-practitioners-mindset}

::: {.callout-tip}
## Install Genesis as you read

This Part III teaches the practice of agentic development as a discipline a human can learn. The agent-side companion that operationalizes the entire block is the open-source skill **[`danielmeppiel/genesis`](https://github.com/danielmeppiel/genesis)**. Install it at the start of this part — it gives you a working reference your agent can load while you work through the chapters.

What Genesis gives you in one sentence: the same primitives, patterns, and disciplines you are about to read, packaged as one Agent Skill that any AGENTS.md-compatible harness (Copilot, Claude Code, Cursor, Codex, OpenCode) can load on demand. When the practitioner block names a pattern (Wave, Panel, Supervised Execution), Genesis names the implementation. When this book argues a discipline (progressive disclosure, plan-write-then-reload, bounded-scope grounding), Genesis enforces it in operational form.

Install:

```bash
gh skill install danielmeppiel/genesis
```

Or follow the per-harness installation notes in the Genesis README. Read this part with Genesis loaded; the chapters call out where Genesis names the same idea, in footnote form.
:::
...
%%| fig-cap: "Decision framework: when to use agents and when to code manually"
%%| fig-alt: "Top-down decision tree flowchart. A task arrives at the first question: 'Can you specify it clearly in under 2 minutes?' Yes leads to 'Is the spec shorter than the code?' which if Yes leads to 'Is the scope bounded?' — Yes results in 'DELEGATE to agent' (green, success). If the spec is not shorter, outcome is 'CODE IT YOURSELF' (neutral). If scope is not bounded, outcome is 'CODE IT YOURSELF' (neutral). If you cannot specify clearly, the question 'Can you split it into agent plus manual parts?' appears — Yes leads to 'SPLIT: bounded parts to agent, judgment parts to you' (green, success); No leads to 'CODE IT YOURSELF' (neutral). Three manual outcomes and two agent outcomes total."
flowchart TD
...
[^ch8-n1]: The limitations of single-case methodology are well-documented. See Flyvbjerg, "Five Misunderstandings About Case-Study Research," *Qualitative Inquiry* 12, no. 2 (2006): 219–245. Our approach follows his argument that a well-chosen single case can be decisive for theory development.
[^ch8-cognitive]: Sweller's Cognitive Load Theory (1988) provides the theoretical basis for why reducing extraneous cognitive load improves performance. Agent-delegated execution reduces extraneous load (build mechanics, file navigation) while preserving germane load (architecture decisions, review).
