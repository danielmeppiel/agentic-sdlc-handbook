...
%%| fig-width: 5.5
flowchart LR
    AUDIT["AUDIT<br/>(understand)"] --> PLAN["PLAN<br/>(scope, waves,<br/>teams)"]
    PLAN --> WAVE["WAVE [0..N]<br/>(parallel agents)"]
    WAVE --> VALIDATE["VALIDATE<br/>(test, spot-check)"]
    VALIDATE -->|All waves done| SHIP["SHIP<br/>(commit, push,<br/>merge)"]
    VALIDATE -->|Next wave| WAVE
    VALIDATE -->|Failure| ADAPT["ADAPT<br/>(diagnose, adjust)"]
    ADAPT --> PLAN
```
...
%%| fig-width: 4.5
flowchart LR
    W0["Wave 0<br/>FOUNDATION"] -->|"No deps"| W1["Wave 1<br/>CORE"]
    W1 -->|"Wave 0 outputs"| W2["Wave 2<br/>MIGRATION"]
    W2 -->|"Wave 1 stable"| W3["Wave 3<br/>POLISH"]
```
...
%%| fig-width: 4.5
flowchart LR
    D["DETECT<br/>Test failure, agent stall,<br/>unexpected conflict"] --> DG["DIAGNOSE<br/>What broke, why,<br/>which wave/task/file"]
    DG --> A["ADJUST<br/>Modify plan: add tasks,<br/>split, reorder waves"]
    A --> E["EXECUTE<br/>Re-run adjusted wave"]
    E -.->|"If new issue"| D
```
