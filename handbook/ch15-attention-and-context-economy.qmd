...
There is a difference between an instruction the model has read and an instruction the model has seen. This chapter is about the gap, why it widens silently, and what closes it.

A staff engineer opens her team's pull-request queue and finds three reviews from the agentic code-reviewer that all missed the same thing: a use of the deprecated `auth_v1` helper inside a new endpoint. The reviewer is supposed to flag exactly this. Its scope-attached instruction file says, in the second paragraph, *Treat any use of `auth_v1` outside `legacy/` as a blocking review comment, never a suggestion.* The file is loaded. She checks the harness's verbose log to be certain. The file is in context. The model just ignored it.
...
Her first instinct is to file a bug against the model vendor. Her second, after talking to a colleague who has read the load-lifecycle chapter, is to ask the harness to tell her how many tokens are in context at the moment of the review and where the `auth_v1` rule sits inside that span. The answer is uncomfortable. The rule is on line 62 of an instruction file that loads into the middle third of a 35,000-token context payload. The model read it. The model did not see it.

A context window is not a budget you spend on more material; it is a working set in which attention is finite, position-sensitive, and degrades non-linearly under load. The disciplines that fall out of this — progressive disclosure, subagent isolation, plan-write-then-reload — are not stylistic preferences. They are the small set of countermeasures that keep the attention economy solvent.[^ch13-genesis] If Chapter 14 was about the deterministic mechanics of *what loads*,[^ch13-load-lifecycle] this chapter is about the probabilistic consequences of *what gets attended to*.
...
This diagnostic loop is part of what Chapter 18 will treat as the agent stack trace.[^ch13-meta-process] When an agentic system misbehaves, the loaded primitive set, the token count, the position of the failing rule, and the conversation length are the first four cells in the trace, before any consideration of the model itself.
...
The mistake progressive disclosure prevents is the architectural-decisions document the team added. The intent — *the reviewer should know the architecture* — was correct. The implementation — load it on every review — was the mistake. The right shape was a skill that reads architectural-decisions only when the diff touches an interface boundary, with the rest of the document staying on disk. Progressive disclosure is not laziness; it is the only way a primitive set can grow without each new primitive making every existing primitive perform worse.

### Lever two: subagent isolation

The second lever is *fresh context window per scoped task*. When work is genuinely independent — a worker porting one module while a peer ports another, a research subagent answering a focused question without inheriting the parent's mid-payload — the discipline is to spawn a child thread with its own context, not to extend the parent. Chapter 17 will treat the multi-agent topology in operational detail.[^ch13-multi-agent] Here it is enough to note that the asymmetry from Chapter 11 — *inference is per-thread; the filesystem is shared* — is what makes this lever work. A child thread starts at a clean head with a U-curve fully its own; the parent passes whatever it needs the child to know via the filesystem (a plan file, a focused brief, a pinned reference) rather than via the parent's polluted context.
...
The third lever is the most counter-intuitive: when a session must be long, *defeat drift by writing the plan to a file and re-reading the file at decision points*. The plan begins at the head of context. Twenty turns later, when it matters, it has slipped into the trough. Re-reading the plan file pulls it back to the tail, where attention is strongest, exactly when the model needs it. The filesystem is the durable memory; the inference is amnesiac; the discipline is to use the durable memory deliberately, not as a backup but as a structural element of how a long session is run. Chapter 18 treats this as a cross-wave attention discipline; here it is the third lever of the economy.[^ch13-meta-process]
...
## Five access mechanisms for the Context layer
...
3. **Web fetch** — pulling an external URL into the window (vendor docs, RFCs, public references). High value when bounded; corrosive when unbounded. The `bounded-scope grounding` discipline introduced in @sec-attention-economy is the load-bearing constraint: name what the fetch is authoritative for *before* you fetch it, drop everything else.
...
**Mechanism choice is a design decision, not an accident.** When the same information could reach the agent through a file, a CLI call, or a web fetch, the choice has consequences. The file is reproducible across runs and survives a session reset. The CLI captures the operator's local environment. The web fetch is fresh but external. The Architecture Decision Matrix in Chapter 5 (and the panel pattern in @sec-multi-agent) treat the choice the same way they treat any other consequential decision — name it, capture the evidence, route it to the right reviewer.

The Panel pattern (named in the Part III preface, operated in @sec-multi-agent, walked end-to-end in the Part III capstone) — *Panel*, *Wave*, *Scatter-Gather*, *Subagent* — is what teams use to *operate* across these mechanisms once the work crosses what a single Skill can hold. Chapter 17 (@sec-multi-agent) is the operational treatment.

---

## What this chapter unlocks

You now have the second half of the load picture. Chapter 14 told you what the harness loads, in what order, with what closure. This chapter told you what the model can attend to once it is loaded, and what disciplines keep the attended-to set aligned with the relevant set. The two together are what *context engineering* is. Most teams that struggle with agentic quality are doing one half and ignoring the other: they tune the load lifecycle and let attention rot, or they obsess over short prompts while the loader silently pulls in half the repository.

The remaining chapters apply this. Chapter 16 will draw the deterministic seam between the harness and the model; the seam is where attention failures must be caught, because the model itself will not catch them. Chapter 17 will use subagent isolation as the structural primitive of multi-agent topology. Chapter 18 will name plan-write-then-reload as one of the rituals that hold a long execution together. The PROSE constraints you saw in Chapter 13 reads differently after this chapter: P (Progressive Disclosure), R (Reduced Scope), O (Orchestrated Composition) are not three preferences, they are the three levers of the attention economy expressed as constraints on the primitives a team writes. The constraint exists because the physics exists.
