...
%%| fig-width: 5.5
%%| fig-height: 8.0
%%{init: {'theme':'base','themeVariables':{'fontSize':'16px','fontFamily':'-apple-system, BlinkMacSystemFont, sans-serif'}}}%%
block-beta
  columns 3
...
**The patterns you already know apply.** A Skill is a Module with a Facade — one entry point, one stable contract, swappable internals. A Persona is a Strategy — same input shape, different evaluation policy. A harness orchestrating a Panel is a Mediator with a Scatter-Gather distribution. The substrate is new; the patterns are not. Chapter 18 (@sec-layered-model) carries the full Rosetta and the four-layer reconciliation.

**The decisions worth making this week are about your registry and your CODEOWNERS, not your agent vendor.** A team that has installed the supply chain can change its mind about harnesses, frameworks, and cloud providers without re-authoring its primitive set; a team that has not, cannot. Chapter 22 (@sec-what-comes-next) carries the five-day plan.
