...
::: {tbl-colwidths="[14,24,18,18,14,12]"}

| Primitive | GitHub Copilot (VS Code) | Cursor | Claude Code | Windsurf | OpenCode |
|-----------|--------------------------|--------|-------------|----------|----------|
| **Instructions** | `.instructions.md` with `applyTo` frontmatter; `copilot-instructions.md` global | `.cursor/rules/*.mdc` with glob frontmatter | `CLAUDE.md` per directory | `.windsurfrules`; cascade rules | `.opencode/instructions.md` |
| **Agents** | `.agent.md` with model + tools | Configurable via rules and agent modes | Via `/commands` and agent configurations | — | — |
| **Skills** | `SKILL.md` directories with examples | Embed in rules with examples | Embed in `CLAUDE.md` sections | Embed in rules | — |
| **Prompts** | `.prompt.md` with execution mode | Embed in rules or `.cursor/prompts/` | Via `/commands` definitions | Flows (different model) | — |
| **Memory** | SQLite database; `.memory.md` as portable context | Notepads; project-level rules | In `CLAUDE.md` sections; persistent memory | In rules | — |
| **Orchestration** | `.spec.md` loaded as context; plan mode artifacts | Loaded as context; composer plans | Loaded as context; plan mode | Loaded as context | Loaded as context |
...
- **Agents, Skills, Prompts, and Hooks** have the deepest native support in GitHub Copilot's `.agent.md`, `SKILL.md`, `.prompt.md`, and hooks system. Other tools achieve similar outcomes through their own configuration mechanisms.
...
**The three major tools now support most primitives natively.** GitHub Copilot, Cursor, and Claude Code each support the majority of the seven primitive types, though file formats and native integration depth differ. Copilot has the most comprehensive native format support. Cursor and Claude Code have expanded rapidly and cover instructions, agents, skills, memory, and orchestration through their own mechanisms. OpenCode, the newest entrant, currently supports mainly instructions. Windsurf covers instructions and basic context loading. The methodology described here is portable across all of them — the knowledge transfers even when the wiring differs.
...
The concepts above are concrete, but seeing them in action makes the feedback loop tangible. The session below is a realistic reconstruction based on the APM PR #394 pattern (documented in Part IV). It shows a developer orchestrating a multi-agent code change across a module, with context files shaping every step.

```
# ──────────────────────────────────────────────────
# WAVE 1: Foundation — Auth module consolidation
# The plan calls for three agents on separate file scopes.
# Each agent gets scoped instructions automatically.
# ──────────────────────────────────────────────────

$ copilot-cli dispatch --scope src/auth/ --plan wave1-auth.yaml
  → Agent reads: .github/copilot-instructions.md       (global rules)
  → Agent reads: src/auth/.instructions.md              (module-scoped)
  → Agent reads: specs/auth-consolidation.prompt.md     (task spec)
  Agent working...  ████████████████████████  done (47s)

$ copilot-cli dispatch --scope src/middleware/ --plan wave1-middleware.yaml
  Agent working...  ████████████████████████  done (31s)

$ copilot-cli dispatch --scope tests/auth/ --plan wave1-tests.yaml
  Agent working...  ████████████████████████  done (52s)

# ──────────────────────────────────────────────────
# REVIEW CHECKPOINT — Developer reviews Wave 1 output
# This is the Reviewer role from Chapter 8.
# ──────────────────────────────────────────────────

$ git diff --stat
 src/auth/provider.py       | 47 +++++++++-------
 src/auth/middleware.py      | 23 ++------
 src/auth/token_service.py   | 18 +++++++
 src/middleware/rate_limit.py | 11 ++--
 tests/auth/test_provider.py | 31 ++++++++++++
 tests/auth/test_tokens.py   | 29 ++++++++++++
...
# ──────────────────────────────────────────────────
# THE CATCH — A silent failure (Anti-Pattern #6 from Ch14)
# Tests pass. But the developer spots something in the diff:
# the agent used the OLD token validation pattern.
#
# Why this matters: the test fixtures also use the old pattern,
# so old code + old fixtures = green tests, wrong behavior.
# This is exactly the "Silent Semantic Failure" pattern —
# the output is locally coherent and globally wrong.
# ──────────────────────────────────────────────────

$ git diff src/auth/token_service.py | head -20
 +    decoded = jwt.decode(token, key, algorithms=["RS256"],
 +                         options={"verify_signature": False})
 #                                  ^^^^^^^^^^^^^^^^^^^^^^^^
 #    WRONG — the legacy pattern. Production requires verify=True.
 #    The agent didn't know because the deprecation wasn't in scope.

# ──────────────────────────────────────────────────
# CONTEXT FIX — Not a code fix. A primitive fix.
# This is the feedback loop: fix the context file so this
# class of error never recurs. (See "The Feedback Loop"
# section earlier in this chapter.)
# ──────────────────────────────────────────────────

$ cat >> src/auth/.instructions.md << 'EOF'

## Token Validation (Critical)
- ALWAYS use jwt.decode() with verify_signature=True.
- The legacy pattern (verify=False) is DEPRECATED.
- If you see verify=False in existing code, flag it — do not copy it.
- See: docs/auth-migration.md for the migration timeline.
EOF

# ──────────────────────────────────────────────────
# RE-DISPATCH — Agent now reads the updated instruction file.
# This is the Escalation Handler role: diagnose the failure,
# fix the context, re-dispatch. (Chapter 8, "Your Three Roles")
# ──────────────────────────────────────────────────

$ copilot-cli dispatch --scope src/auth/ --plan wave1-auth.yaml --retry
  → Agent reads: src/auth/.instructions.md  (NOW INCLUDES token rule)
  → Agent reads: specs/auth-consolidation.prompt.md
  Agent working...  ████████████████████████  done (39s)

$ git diff src/auth/token_service.py | head -10
 +    decoded = jwt.decode(token, key, algorithms=["RS256"],
 +                         options={"verify_signature": True})
 #    ✓ Correct pattern. The updated instruction prevented the mistake.

# ──────────────────────────────────────────────────
# WAVE 2: Dependent work — the middleware integration
# Now that auth is solid, dispatch the next wave.
# This is orchestrated composition (Chapter 10, Constraint O):
# Wave 2 agents consume Wave 1's committed output as context.
# ──────────────────────────────────────────────────

$ git add -A && git commit -m "feat(auth): consolidate provider, add token service"
$ copilot-cli dispatch --scope src/api/ --plan wave2-api-integration.yaml
  Agent working...  ████████████████████████  done (44s)

$ pytest tests/ -q
 31 passed in 4.2s

# All green. PR ready for human review.
# Total: ~5 min hands-on, ~3 min agent execution, 1 context fix.
# The context fix is the lasting value — it prevents the same
# mistake on every future auth task, for every developer.
```

Three things to notice.

**The developer never wrote application code.** Every line of `src/` was produced by agents. The developer's contributions were the plan, the review, and the context fix. This is the role shift Chapter 8 described: architect, reviewer, escalation handler.

**The fix was to the instruction file, not the generated code.** A developer working without instrumentation would have fixed `token_service.py` and moved on. The same mistake would recur next week, with a different agent, on a different task. Fixing the context file (the scoped `.instructions.md`) prevents the entire class of error, permanently. This is the feedback loop that makes instrumentation compound over time.

**The silent failure was caught by a human, not a test.** The tests passed because the test fixtures encoded the same wrong assumption the agent did. This is Anti-Pattern #6 from Chapter 14: *Silent Semantic Failure*. Automated checks are necessary but not sufficient. The developer's domain knowledge (knowing that `verify=False` is deprecated) was the safety net. Agent-augmented development does not eliminate the need for human judgment; it changes where that judgment is applied.
