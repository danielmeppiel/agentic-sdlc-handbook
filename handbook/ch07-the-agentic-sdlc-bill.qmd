...
: The four spend pools, each funded and metered by a different rule. The cheaper pools run open; only the deliberate bets — frontier exploration and metered at-scale runs — carry a gate. {#tbl-spend-pools tbl-colwidths="[20,30,28,22]"}
...
```{mermaid}
%%| label: fig-workflow-lifecycle
%%| fig-cap: "The agentic workflow lifecycle. One exploration is paid once, at frontier cost — the orange stage, spend to learn. The five stages that follow turn that exploration into a governed, reusable workflow whose every rerun pays the learning back (green). DISCOVER closes the loop: real usage reveals the next gap worth exploring. The bill is front-loaded once; the value compounds on every turn after."
%%| fig-alt: "A six-node clockwise cycle. One node is highlighted orange and labelled spend to learn: EXPLORE, on a frontier model at capped cost. The other five nodes are green and labelled spend that pays back: CODIFY (persist as a reusable skill), PUBLISH (portal skill, policy-gated), CONSUME (pull by manifest), RUN AND MONITOR (cost per outcome), and DISCOVER (usage reveals the next gap). A return edge labelled next exploration runs from DISCOVER back to EXPLORE, closing the loop and showing that reuse feeds the next round of learning."
%%| fig-width: 5.4
%%| fig-height: 5.2
flowchart TB
