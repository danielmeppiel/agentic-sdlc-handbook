...
**The quality discount.** Raw speed metrics count code produced. They don't count code reworked, reverted, or debugged downstream. As established in Chapter 1, 30–60% of agent-generated code on complex tasks requires significant rework — a figure supported by Stack Overflow developer surveys and GitClear code quality analyses.[^ch3-rework] This means architectural changes, convention fixes, and security remediation, not cosmetic edits. If an agent produces a function in 30 seconds that takes 20 minutes to correct, the net productivity may be negative. Measuring production without measuring rework is measuring revenue without measuring returns.[^ch3-attention-economy]
...
The visible cost — and the only one vendors emphasize. Pricing models vary across tools and are evolving rapidly, but as of early 2025, representative price points for the major agentic coding platforms illustrate the range[^ch3-pricing]:
...
[^ch3-pricing]: Prices as of March 2025. See [GitHub Copilot plans](https://docs.github.com/en/copilot/about-github-copilot/plans-for-github-copilot), [Cursor pricing](https://cursor.com/pricing), [Anthropic pricing](https://anthropic.com/pricing). These change frequently; verify current rates before budgeting.
...
| Defect reduction | Review rejection rate, post-deploy defects | Months 6–9 † | 15–30% reduction in convention violations[^ch3-convention] † |
...
```{mermaid}
%%| fig-width: 4.5
%%| fig-cap: "Cost-benefit trajectory over a 12-month adoption period"
%%| fig-alt: "An XY chart titled 'Adoption J-Curve: Expect the Valley' showing net productivity impact over 12 months. The line starts at 0 in month 0, dips to negative 25 around month 2 representing the investment valley, crosses back to positive around month 5, then steadily climbs to approximately 52 by month 12 as context compounds and returns accelerate."
...
The point is not the specific numbers — it is the shape. The business case is robust across a wide range of rework assumptions. Even at the conservative end, the investment breaks even within a year. But the difference between 20% and 60% rework is a 2× spread in annual value. This is why context engineering — which directly reduces rework — is the highest-leverage investment in the entire adoption plan.
...
::: {.callout-note title="Adoption Cost Framework"}
...
*The pilot phase is designed to be reversible. If it doesn't work for your codebase, the only cost is a few weeks of reduced velocity. The instrumentation investment (Chapter 10) pays dividends even without agentic workflows.*
:::
...
[^ch3-convention]: Based on the author's observations across instrumented projects during early adoption. Chapter 10 reports a wider range (40–60% violation rate dropping to under 10%) for teams with mature instrumentation and comprehensive context architecture. The difference reflects adoption stage: this chapter's 15–30% improvement represents early-adoption teams with basic context engineering; Chapter 10's figures reflect fully instrumented codebases.

[^ch3-attention-economy]: The rework rate has a structural cause that Block 2 names directly: agents operate against a finite *attention economy* — the working set of tokens an LLM can hold in focus on any single invocation. When too much irrelevant context loads, the relevant context degrades, and the agent produces plausible-but-wrong output that has to be reworked downstream. Chapter 13 makes this mechanism explicit; Chapter 12 covers the load lifecycle that decides what enters the working set in the first place. The business-case point is that rework rates are not a tooling defect to be patched — they are a function of how disciplined the context architecture is.
