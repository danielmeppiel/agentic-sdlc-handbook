...
The story below uses three filename shapes that recur across every harness. If they are new, here is enough to read on without losing the thread. Chapter 11 (@sec-instrumented-codebase) catalogues all seven primitive types with full examples and design tests.
...
**Chapter 13** treats the load lifecycle in detail. You will learn what to look for in the harness's verbose output, how to compute the transitive closure of files that load when a primitive activates, and why your skill is silent when a parent rule file overflows the context budget.[^ch9-substrate2]
