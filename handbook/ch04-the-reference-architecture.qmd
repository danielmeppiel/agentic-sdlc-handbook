...
For leaders, the seam is the architectural fact that explains why "the model got better" is rarely the right investment. The agent's quality at the seam is bounded by how well the platform side enforces its half of the contract. A more capable model crossing a poorly designed seam produces more articulate failures, not fewer. Block 2's Chapter 14 operates the seam in detail; the planning point here is that every governance, audit, and safety conversation in the rest of this block is, structurally, a conversation about how the seam is enforced in your organization.
...
The three lenses do not contradict each other. They project the same architecture onto different decision surfaces. The Three-Layer lens answers organizational questions. The four-layer substrate lens answers engineering questions. The seven-layer computing-stack lens answers procurement and strategy questions. Block 2 reconciles all three explicitly; for the strategic decisions this chapter sets up, the computing-stack lens is the one to use.
