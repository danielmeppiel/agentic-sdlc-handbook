...
flowchart TD
    ...
    Q1 -- Yes --> FIX1["Model capability<br/>Upgrade model or<br/>manual fallback"]
    Q1 -- No --> Q2{Tests fail?}

    Q2 -- Yes --> Q2a{Agent's code<br/>or pre-existing?}
    Q2a -- "Agent's" --> FIX2["Task too broad?<br/>Context stale?<br/>Constraint dropped?"]
    Q2a -- Pre-existing --> FIX2b["Separate issue<br/>Don't fix here"]

    Q2 -- No --> Q3{Follows your<br/>conventions?}

    Q3 -- No --> FIX3["Primitive gap<br/>Rule missing or<br/>wrong scope?"]
    Q3 -- Yes --> Q4{Integrates correctly<br/>with system?}

    Q4 -- No --> FIX4["Architecture issue<br/>Wrong interfaces<br/>or boundaries?"]
    Q4 -- Yes --> FIX5["Probably fine<br/>Verify edge cases"]
