...
%%| fig-width: 8.5
flowchart TD
    START(["Something went wrong<br/>with agent output"]) --> Q1{Code syntactically<br/>wrong?}

    Q1 -- Yes --> FIX1["Model capability issue<br/>Stronger model, better examples,<br/>or do it manually"]
    Q1 -- No --> Q2{Tests fail?}

    Q2 -- Yes --> Q2a{Agent's code<br/>or pre-existing?}
    Q2a -- "Agent's" --> FIX2["Check: task too broad?<br/>Context stale?<br/>Constraint dropped?"]
    Q2a -- Pre-existing --> FIX2b["Separate issue<br/>Do not fix in this session"]

    Q2 -- No --> Q3{Follows your<br/>conventions?}

    Q3 -- No --> FIX3["Primitive issue<br/>Rule missing? Wrong scope?<br/>Too much context noise?"]
    Q3 -- Yes --> Q4{Integrates correctly<br/>with system?}

    Q4 -- No --> FIX4["Architectural issue<br/>Right interfaces visible?<br/>Module boundaries respected?"]
    Q4 -- Yes --> FIX5["Probably fine<br/>Verify edge cases: error paths,<br/>null inputs, concurrency, auth"]
