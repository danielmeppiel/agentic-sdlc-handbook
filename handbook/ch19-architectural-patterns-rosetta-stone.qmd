...
Read the layered model in §1 first. After that, the per-layer catalogues (§3–§6, with a short Assembly-layer catalogue at §3′) are independently scannable. The decision matrix in §7 indexes the catalogue by problem; §8 names what this chapter deliberately leaves out.
...
Two pieces of vocabulary recur in the cost-aware entries below and are worth fixing before you reach them. A **cacheable prefix** is the stable head of the context window — system preamble, tool definitions, scope-loaded rules, agent source — that a harness can bill at a fraction of full input on repeat turns. A **cache invalidator** is anything placed in that head which mutates per turn — a timestamp, an ephemeral ID, a mid-session tool addition, a model switch — and so re-prices the entire prefix at full input on the next turn. The attention-economy chapter (@sec-attention-economy) develops the same layout decision in its other currency; the cost entries here track the dollar ledger, that chapter tracks the attention one, and the two optima coincide.
...
## 3′. Assembly layer {#sec-assembly-patterns}

The Assembly layer is named in §1 but, until the cost dimension forced the question, had no catalogue of its own: how content is laid out in the context window was treated as a load-lifecycle concern (Ch13, @sec-load-lifecycle) rather than a pattern with a trade-off. Prompt-prefix caching changes that. Once a harness bills a cached prefix at a fraction of full input, *where* you place each piece of content becomes an architectural decision with a dollar consequence — and it earns an entry.

| Agentic name | Classical analogue (rigour) | Intent | When to apply | Consequences | Cross-refs |
|---|---|---|---|---|---|
| **Cache-Aware Prefix** | Cache-Aside [^kleppmann2017] — *precise* / Append-Only Log — *partial* | Lay out the context window so stable content (system preamble, tool definitions, scope-loaded rules, agent source) sits at the prefix head and volatile content (current turn, in-flight diff, fresh tool results) at the suffix tail, making the prefix cacheable across turns at a fraction of full input. | Whenever the harness exposes prompt-prefix caching as a billing unit (every major foundation today) and a session runs more than a turn or two against the same primitive set. | Anything mutable placed in the head — timestamps, ephemeral IDs, the conversation's own scrollback — is a cache invalidator that re-prices the whole prefix at full input. Layout becomes a load-bearing public contract, not a cosmetic choice. | Ch13 (@sec-load-lifecycle); Ch14 (@sec-attention-economy); Lockfile (§6) |

Cache-Aware Prefix is the first pattern to demand an Assembly-layer catalogue, and likely not the last: as harnesses expose more of the assembly seam as billable surface, the layout of the context window will accrete patterns the way composition did. For now it carries a single entry, because that entry pays its rent (§8) and a speculative second would not.
...
| **Model Router** | Strategy [^gof1994] — *partial* / Content-Based Router [^hohpe2003] — *precise* | Classify a task's complexity in a cheap preliminary turn and dispatch the substantive work to a right-sized model rather than running every task on one class. | When the workload mixes tasks of materially different complexity (deep planning vs. lint vs. classification) and the foundation layer exposes ≥2 model classes with a 5×+ cost spread. | The classification turn is itself a cost and latency tax; routing must be cache-aware (a mid-trajectory model switch is a cache invalidator and pays full input on the prefix next turn); the routing rules are a public contract whose drift produces silent regressions. | Ch10 (@sec-runtime-machine); Gradient Workflow; Cache-Aware Prefix (§3′) |
| **Gradient Workflow** | Tiered Architecture [^posa1996] — *partial* / Stratified Pipeline — *weak* | Compose a workflow whose stages run on differently sized model classes — a heavy planner, a mid-tier implementer in the fan-out, a lightweight reviewer or triage layer — rather than one class throughout. | When a workflow has a small number of consequential decisions (heavy class), many mechanical edits (mid-tier), and high-volume filtering or classification (lightweight), and the foundation layer offers all three. | Each tier boundary is a model switch and so a cache invalidator within a trajectory; the topology must place switches at thread or wave boundaries — where a new context starts anyway — to avoid paying full input on a warm prefix. Composes with Panel (mid-tier lenses, heavy synthesiser) and Wave (heavy planner stage, mid-tier execution); without that discipline it degrades into "use a cheaper model and hope". | Ch16 (@sec-multi-agent); Model Router; Wave |
...
A note on Model Router vs Gradient Workflow. The two are easily conflated and they are not the same axis. A **Model Router** decides *per task, at dispatch time* — it classifies the work in front of it and picks a model. A **Gradient Workflow** decides *per stage, at design time* — the topology fixes which tier each stage runs on before any task arrives. Router answers "which model for this task?"; Gradient answers "which model for this stage?" They compose: a single stage of a Gradient Workflow can itself route.

A note on Wave. Wave is Pipes-and-Filters [^parnas1972] applied to LLM stages, with the addition that each filter's output is *durable* (a plan artifact) and each gate is a *deterministic* check, not a model call. The "filter" metaphor undersells the durability requirement; in agentic contexts, Wave only works because each stage's output can be re-read in a fresh thread. That same durability is what lets a **Gradient Workflow** (§4) switch model classes at a wave boundary without paying a cache penalty: the boundary is already a cold-context hand-off, so the model switch costs nothing the hand-off did not already cost.
...
| **Tool Subset** | Facade [^gof1994] — *partial* / Interface Segregation [^martin2017] — *precise* | Expose to the model only the subset of tools the current phase or sub-task actually needs, rather than the full catalogue; the substrate variant consolidates several narrow tools behind a single code-execution surface that composes them inline. | When tool-definition tokens grow to dominate the prefix (a common threshold is ~20 tools loaded every turn), or when the same tools load identically across phases that need disjoint subsets. | The subset becomes a phase-scoped public contract; mid-session catalogue mutation is a cache invalidator and must be avoided once a trajectory is warm. The code-execution variant trades per-tool schema rigour for prefix compression — Schema-Checked Transformer moves *inside* the tool rather than at the call seam. A deliberate trade-off, not a free lunch. | Ch14 (@sec-attention-economy); Supervised Execution; Schema-Checked Transformer |
...
| Assembly | A session that runs more than a turn or two against the same primitives | **Cache-Aware Prefix** | Prefix layout becomes a public contract; no timestamps or mutable IDs in the head | Every turn pays full input on the entire prefix |
...
| Dispatch | Mixed-complexity workload with a 5×+ cost spread across model classes | **Model Router** | Classification-turn cost; routing rules are a public contract | Single-class cost floor; overspend on trivial work or underpower hard work |
| Dispatch | A workflow with stratifiable stages (planning / execution / triage) | **Gradient Workflow** | Tier switches must land at thread or wave boundaries to preserve cache | Flat model assignment; heavy model on cheap work |
...
| Boundary | A tool catalogue large enough to dominate the prefix, with disjoint per-phase usage | **Tool Subset** | Subset is a phase-scoped contract; mid-session mutation is a cache invalidator | Tool-definition tax compounds with turn count; prefix crowds out grounding |
...
[^genesis]: Where pattern names in this chapter overlap with Genesis's agent-side codification (Tier-2 design patterns and Tier-3 architectural patterns in the `danielmeppiel/genesis` repository), the human-readable name appears in the table; Genesis's internal codes (used by the agent's dispatch logic) are not load-bearing in this chapter. Genesis is one source of pattern naming the author has drawn from; the analogies to GoF and to distributed-systems literature are this book's contribution. The cost-economics entries added here — Cache-Aware Prefix (§3′), Model Router and Gradient Workflow (§4), and Tool Subset (§5) — carry the handbook-side names for shapes Genesis codifies as agent-loadable cost patterns in the same [`architectural-patterns.md`](https://github.com/danielmeppiel/genesis/blob/613f2a64a4a193cacd45fd4439a093044ae3178d/skills/genesis/assets/architectural-patterns.md) catalogue. Three further Genesis cost additions are deliberately *not* catalogued here: output-side prompt thrift (a coding discipline, codified in agent instructions), a per-harness effort governor (a vendor-specific configuration surface, not a portable pattern), and a cost-prune refactor (a refactor trigger). None is an architectural pattern in this chapter's sense — they are coding disciplines and configuration surfaces, the genre an inclusion bar exists to turn away before §8's three pattern-genres are even reached; recording why is half the point.
...
[^martin2017]: Robert C. Martin. *Clean Architecture: A Craftsman's Guide to Software Structure and Design.* Prentice Hall, 2017. Source for the Interface Segregation Principle, the precise frame for Tool Subset.
