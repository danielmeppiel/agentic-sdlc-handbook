...
**Load mode:** eager preload, scoped by `applyTo` glob. The harness reads matching instruction files into context whenever a thread touches a path the glob covers — Maya's `api.instructions.md` from Chapter 11 is the canonical example.
...
**Load mode:** dispatcher-mediated. The persona file isn't preloaded; a parent thread (or the developer) routes a sub-thread to it through the harness's delegation tool. The selected agent's frontmatter then governs the new thread's model, tool set, and instructions. Chapter 17 builds on this seam.
...
The design test this section just defined is the same test I had to apply to the design discipline itself before I packaged it; the result is the agent-side companion Part III pointed at.[^ch10-genesis-skill]
...
**Load mode:** eager preload, persistent. The harness reads the memory file (or the equivalent store) at every session start — the file is the only thing that survives the per-thread amnesia identified in Chapter 11. Memory is what makes a session that ends at 6pm useful to a session that starts at 9am tomorrow.
...
**Turn 0 — The messy start.** The session opens with the developer pasting raw terminal output of an authentication failure and role-casting the agent in a single breath. This is the Architect role from Chapter 10 — not writing a plan document, but framing the problem and the expertise needed to solve it:
...
That last sentence is this chapter's entire thesis in twelve words. The developer didn't just fix the code — they fixed the context file so every future agent inherits the correction. This is the *Silent Semantic Failure* anti-pattern from Chapter 20 caught and resolved at the primitive layer.
...
**Turn 71 — Granular review, then delegation.** The developer shifted into the Reviewer role (Chapter 10) at its most specific, then handed off to specialists:
...
**Chapter 13 — The PROSE Constraints (@sec-prose).** How to *write* the prose inside these files so it actually steers the model. Each PROSE constraint — Progressive Disclosure, Reduced Scope, Orchestrated Composition, Specification, Explicit Hierarchy — is a normative rule for the body of a primitive. Chapter 13 is the writing chapter; this one was the typing chapter.

**Chapter 14 — The Load Lifecycle (@sec-load-lifecycle).** How the harness *loads* these files. The five load modes named in this chapter — eager preload, lazy on-demand, dispatcher-mediated, user-invoked, event-driven — get their mechanics there: load order, transitive closure, the dispatcher's binding window, why a correctly-placed skill stays silent when a parent instruction overflows the context budget. Chapter 14 is the chapter you reach for when a primitive does not bind.

**Chapter 15 — Attention and Context Economy (@sec-attention-economy).** What these files *cost* once loaded. Loading is deterministic; attention is not. A 40-line instruction file and a 400-line one bind identically and behave nothing alike. Chapter 15 is the physics — why context is a working set rather than a budget, why doubling input length more than halves output quality past a threshold, why progressive disclosure is the price of admission rather than polish.

This chapter showed you what to build. Chapter 13 shows you how to write it. Chapter 14 shows you when it loads. Chapter 15 shows you what it costs.

[^ch9-lint]: Static analysis tools (ESLint, Pylint, Ruff) catch syntactic violations but not semantic ones — an agent can pass all linters while violating domain conventions. The instruction files complement, rather than replace, automated linting. See also the discussion of Silent Semantic Failure in Chapter 20.
