...
%%| fig-width: 5.5
flowchart TD
    W0["Wave 0: FOUNDATION<br/>No dependencies, fully parallel"]
    W0A["Agent A: type definitions"]
    W0B["Agent B: utility module"]
    W0C["Agent C: configuration"]
    W0 --- W0A & W0B & W0C
    CHECK0{"All complete,<br/>tests pass"}
    W0A & W0B & W0C --> CHECK0
    W1["Wave 1: CORE CHANGES<br/>Depends on Wave 0"]
    CHECK0 --> W1
    W1D["Agent D: module migration<br/>(uses new types)"]
    W1E["Agent E: API consolidation<br/>(uses new utilities)"]
    W1F["Agent F: auth refactor"]
    W1 --- W1D & W1E & W1F
    CHECK1{"All complete,<br/>tests pass"}
    W1D & W1E & W1F --> CHECK1
    W2["Wave 2: INTEGRATION<br/>Depends on Wave 1"]
    CHECK1 --> W2
    W2G["Agent G: wiring + imports"]
    W2H["Agent H: test updates"]
    W2 --- W2G & W2H
```
...
%%| fig-width: 5.5
flowchart TD
    A1["1. ASSESS<br/>Scope: single-agent<br/>vs. multi-agent"] --> A2["2. SPECIALIZE<br/>Choose team pattern"]
    A2 --> A3["3. PARTITION<br/>Files across agents<br/>(one file, one agent per wave)"]
    A3 --> A4["4. ORDER<br/>Waves by dependency"]
    A4 --> A5["5. DISPATCH<br/>Each wave in parallel"]
    A5 --> A6{"6. VALIDATE<br/>Test, commit,<br/>checkpoint"}
    A6 -->|"Pass"| A5
    A6 -->|"Fail"| DX["DIAGNOSE"]
    DX -->|"L1/L2"| RETRY["Retry with<br/>refined prompt"]
    DX -->|"L3/L4"| HUMAN["Human decides,<br/>adapt plan"]
    RETRY --> A5
    HUMAN --> A4
```
