...
- The technical mechanism that makes Phase 3 sustainable is a discipline that Block 2 names *capability gating*: not every agent invocation gets the full available toolset and context. The load lifecycle gates which capabilities are loaded eagerly, which are scope-attached to a specific task, which are loaded lazily on demand, and which are mediated by a dispatcher. Phase 3 organizations that succeed treat capability gating as the lever they tune, not the toolset itself. Block 2 operates these binding modes in detail; for now, the planning point is that "everyone gets every tool always" is a Phase 1 starting position, not a Phase 3 endpoint.
...
This chapter closes Block 1. If you also write code — or need to understand what your practitioners will be doing — Block 2 begins with Chapter 8, where context engineering moves from strategy to implementation. The frameworks here are the "what" and "when." Block 2 is the "how." Practitioners install the agent-side artefacts (Genesis, the methodology bundle this book documents) at the start of Block 2; leaders who want to see the artefacts firsthand can skim Chapter 8's opening, but doing so is not required to set the strategy this chapter lays out.
