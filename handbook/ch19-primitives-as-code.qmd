...
[^ch19-genesis]: The package shape this section walks through is the live shape of `apm install` [`danielmeppiel/genesis`](https://github.com/danielmeppiel/genesis). Its [`apm.yml`](https://github.com/danielmeppiel/genesis/blob/main/apm.yml) declares `name: genesis` and `version: 0.1.0`, with a `devDependencies` block holding a single `path: ./dev/skills/genesis-evals` entry for maintainer-only infra deliberately kept outside the published bundle. Its [`apm.lock.yaml`](https://github.com/danielmeppiel/genesis/blob/main/apm.lock.yaml) records `package_type: claude_skill` and `is_dev: true` for the local dev skill resolved under that block — one author's stack, inspectable; inspect it, do not inherit it.
